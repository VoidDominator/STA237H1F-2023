...
# dots <- numeric(reps)
# for (i in 1:reps) {
#   temp_flips <- sample(dice_outcomes, 2, replace = TRUE)
#   dots[i] <- temp_flips
# }
#
# nC <- 0
# nDinC <- 0
# for (i in 1:reps) {
#   if ((dots[i][0] == 1 & dots[i][1] != 1) | (dots[i][0] != 1 & dots[i][1] == 1)) {
#     nC <- nC + 1
#     if (sum(dots[i]) < 6) {
#       nDinC <- nDinC + 1
#     }
...
```{r}
# 5ci. count
# choose(6, 2) # without replacement
6^2 # with replacement

# 5cii. simulate
digits <- 1:6
# set.seed(1008866817)
# sample(digits, 2, replace = FALSE) # without replacement
set.seed(1008866817)
